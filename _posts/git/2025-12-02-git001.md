...
위 방법을 사용하면 하나의 커밋 단위로 묶이게 되므로 커밋 메시지가 일괄로 전송됩니다. 따라서 커밋 메시지를 세분화해야 할 때는 좋은 선택이 아닙니다.
...
-m 대신 -M을 사용하면 이름이 겹치는 브랜치가 있어도 강제로 덮어씌워 변경합니다. 따라서 -m을 사용해 겹치는 이름이 있는지 확인 후 변경하는 것이 일반적입니다. -M을 사용한 경우는 처음 원격과 로컬을 연결할 때 git branch -M main(현재 브랜치 master를 main으로 변경) 명령어밖에 본 적이 없네요.
...
커밋 되돌리기 1 :
    ...
    git reset --soft HEAD^
    -> 마지막 커밋을 삭제하고, 그 커밋의 작업물 내용이 그대로 stage에 올라간 상태로 남습니다.
