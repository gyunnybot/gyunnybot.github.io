...
원격 저장소 조회하기 :
    ...
    git remote -v(url도 같이 보고싶을 때 추가)
...
커밋 되돌리기 1 :
    ...
    -> 마지막 커밋을 삭제하지만, 그 커밋에서 변경된 내용은 작업 디렉토리(workdir)에 "수정됨" 상태로 남습니다.
        (index(스테이징)은 비워짐)
...
혼자 하는 토이 프로젝트, 연습 :
    그냥 git checkout main && git merge my_branch && git push 정도면 충분합니다.

팀 협업, 코드 리뷰가 필요한 과제/프로젝트 :
    브랜치를 새로 딴 후 main에 push -> PR 생성 -> 리뷰 -> github PR에서 Merge 완료 -> 로컬의 main 브랜치에서 pull(원격 저장소와 동기화) -> merge된 브랜치 삭제
이 흐름으로 가는 것이 표준입니다.
...
rebase:
    ...
    base 브랜치의 HEAD 위에 합치려는 브랜치의 커밋 기록을 쌓아 HEAD가 최신 커밋을 가리키게 하는 방법입니다.
