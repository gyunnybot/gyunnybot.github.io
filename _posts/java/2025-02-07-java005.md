...
특별히 tryLock() 메서드는 락을 즉시 획득할 수 없는 상황이라면 대기하지 않고 스레드를 곧바로 false로 반환합니다. 또한 tryLock()은 시간(ms 단위)을 매개 변수의 인자로 받을 수 있는데, 해당 시간 동안은 락을 획득하려 시도하다가 시간이 지나면 더 이상 기다리지 않고 false를 반환 후 이후 코드를 계속 실행하게 할 수도 있습니다.
