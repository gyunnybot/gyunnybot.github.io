...
한편, 스레드가 모니터 락(뒤에서 설명한다)을 대기할 때에는 BLOCKED 상태가 되며, wait()를 자발적으로 호출하면 WAITING으로 스레드 상태를 변경할 수도 있다. 두 대기 상태는 차이점이 분명 존재하지만, OS 관점에서는 전부 BLOCKED 상태로 인식된다.

마지막으로 스레드는 sleep() 메서드를 사용해 시간이 있는 대기 상태인 TIMED_WAITING으로 변경할 수도 있으며, 모든 프로세스 작업이 완료되면 TERMINATED로 상태를 변경한 후 스레드를 종료한다.

앞서 잠깐 언급한 OS 관점에서의 프로세스 상태와 매핑을 시켜보면, CPU가 멀티태스킹으로 프로세스를 처리할 때 프로세스는 READY와 RUNNING 상태를 짧은 시간 단위로 번갈아가며 수행되지만, Java는 이 둘을 구분하지 않고 RUNNABLE로 통합해 표현한다. 즉, Java 스레드의 RUNNABLE은 OS 관점에서는 READY 상태일 수도 있고 RUNNING 상태일 수도 있는 것이다.

또한 Java의 BLOCKED, WAITING, TIMED_WAITING 상태는 운영체제 프로세스 관점에서 모두 BLOCKED로 간주된다. 운영체제의 BLOCKED 상태는 일반적으로 I/O 혹은 특정 이벤트를 기다리는 상태를 의미하는데, 자바 스레드가 WAITING이나 TIMED_WAITING 상태에 있는 것 역시 \'CPU가 필요 없고, 어떤 이벤트를 기다리는(Blocked-like)\' 상태라는 점에서 OS가 동일한 CPU 상태라고 인식한다. 따라서 자바 스레드의 BLOCKED, WAITING, TIMED_WAITING 상태는 운영체제가 BLOCKED 상태로 뭉뚱그려 인식한다.
...
락을 가진 스레드가 synchronized 블록/메서드를 빠져나와 락을 놓을 때, 비로소 JVM이 대기 중인 스레드 하나를 골라 RUNNABLE로 상태를 바꿀 수 있으며, 대기 중인 스레드는 인터럽트가 걸려도 락을 얻기 전까지 그대로 BLOCKED 상태를 유지한다. 인터럽트 처리 또한 락을 얻은 이후 수행한다.
...
이 방식에서 사용되는 락은 synchronized에서 사용하는 모니터 락이 아닌, Lock 인터페이스가 내부적으로 정의한 라이브러리 수준의 락이다. 따라서 대기 장소도 다른데, 락 획득에 실패한 스레드는 WAITING 상태로 변경되어 스레드 큐에서 대기하게 된다.

특별히 tryLock() 메서드는 락을 즉시 획득할 수 없을 경우, 대기하지 않고 즉시 false를 반환 후 스레드를 종료시킨다. 시간(ms)을 인자로 받을 수 있는데, 이때는 설정된 시간 동안 락 획득을 시도한 후 시간이 초과되면 더 이상 대기하지 않고 false 반환 후 종료한다.
...
한편, 락을 획득한 상태지만 코드 흐름상 대기가 필요한 경우도 있다. 이때는 Object 클래스의 wait() 메서드를 호출해 모니터 락을 반납하고, 스레드 대기 집합으로 이동시켜 WAITING 상태로 만들 수도 있다. 이후 락을 획득한 다른 스레드가 notify()나 notifyAll() 메서드를 호출하면, 스레드 대기 집합에 있던 스레드가 깨워지며 다시 실행 흐름으로 복귀한다.

정리하면, 임계 영역에 접근할 수 없는 상황에서 스레드는 우선적으로 모니터 락을 획득하기 위해 락 대기 집합(synchronized)에서 BLOCKED 상태로 대기한다. 차후에 락을 획득하여 스레드가 실행될 수도 있고, 락을 획득하더라도 사용자가 상황에 따라 락을 반납 후 스레드 대기 집합으로 옮길 수도 있다. 이때는 WAITING 상태로 대기하며, notify()에 의해 깨워져 다시 실행될 수 있다.