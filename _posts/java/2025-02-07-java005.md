...
스레드의 상태는 크게 NEW, RUNNABLE, BLOCKED, WAITING, TIMED_WAITING, TERMINATED 상태로 분류할 수 있다. NEW는 스레드가 생성되었지만 start() 메서드가 호출되지 않은 상태다. start() 메서드를 호출하면 Runnable 내부의 run() 메서드가 실행되면서 스레드가 활성화되고, 멀티태스킹 환경에서 여러 스레드가 RUNNABLE 상태로 수행된다.

한편, 스레드는 sleep() 메서드를 사용해 시간이 있는 대기 상태인 TIMED_WAITING으로 변경할 수도 있으며, 모든 프로세스 작업이 완료되면 TERMINATED로 상태를 변경한 후 스레드를 종료한다.

굳이 프로세스 상태와 매핑을 시켜보면, CPU가 멀티태스킹으로 프로세스를 처리할 때 READY와 RUNNING 상태가 짧은 시간 단위로 번갈아가며 교체되지만, Java는 이 둘을 구분하지 않고 RUNNABLE로 통합해 표현한다. 다시 말해 Java 스레드의 RUNNABLE은 OS 관점에서는 READY 상태일 수도 있고 RUNNING 상태일 수도 있는 것이다.

반면 Java의 BLOCKED, WAITING, TIMED_WAITING 상태는 운영체제 프로세스 관점에서 모두 BLOCKED로 간주된다. OS는 자바 스레드가 모니터 락을 기다리는지(waiting for lock), 조건 변수를 기다리는지(wait), 타이머를 기다리는지(sleep) 구분하지 않고, \'CPU가 필요 없는 대기 중인 스레드\'로 취급하기 때문이다.

운영체제의 BLOCKED 상태는 일반적으로 I/O 혹은 특정 이벤트를 기다리는 상태를 의미하는데, 자바 스레드가 WAITING이나 TIMED_WAITING 상태에 있는 것 역시 \'CPU가 필요 없고, 어떤 이벤트를 기다리는(Blocked-like)\' 상태라는 점에서 OS의 BLOCKED와 의미가 동일하다. 따라서 자바 스레드의 BLOCKED, WAITING, TIMED_WAITING 상태는 운영체제가 BLOCKED 상태로 뭉뚱그려 인식한다.
...
오직 락을 가진 스레드만이 synchronized 블록/메서드를 빠져나와 락을 놓을 때 JVM이 대기 중인 스레드 하나를 골라 RUNNABLE로 상태를 바꿀 수 있으며, 다른 외부 스레드에서 인터럽트를 걸어도 락을 얻기 전까지 해당 스레드는 그대로 BLOCKED 상태를 유지한다.
...
정리하면, 임계 영역에 접근할 수 없는 상황에서 스레드는 우선적으로 모니터 락을 획득하기 위해 락 대기 집합(synchronized)에서 BLOCKED 상태로 대기한다. 차후에 락을 획득하여 스레드가 실행될 수도 있고, 락을 획득하더라도 사용자가 상황에 따라 락을 반납 후 스레드 대기 집합(lock())으로 옮길 수도 있다. 이때는 WAITING 상태로 대기하며, notify()에 의해 깨워져 다시 실행될 수 있다.