...
특별히 tryLock() 메서드는 락을 즉시 획득할 수 없는 상황이라면 대기하지 않고 종료 후 false를 반환합니다. tryLock()은 시간(ms 단위)을 인자로도 받을 수 있는데, 해당 시간 동안은 락을 획득하려 시도하다가 시간이 지나면 더 이상 기다리지 않고 false를 반환합니다.
