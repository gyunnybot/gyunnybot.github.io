...
Java는 프로세스가 종료될 때 자원 정리나 로그 기록과 같은 종료 작업을 마무리 할 수 있는 ShutdownHook 기능을 제공한다. 전체 프로세스를 종료할 때 정리하려는 작업이나 자원을 ShutdownHook 객체의 인자로 등록한 후, \'Runtime.getRuntime().addShutdownHook()\' 경로를 통해 안정적으로 작업을 종료하거나 자원을 회수할 수 있다.
