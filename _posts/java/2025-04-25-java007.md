...
## 소켓 프로그래밍
여러 컴퓨터 간 통신을 네트워킹이라고 한다. 서로 다른 컴퓨터의 프로세스가 통신할 때는 네트워크를 사용하며, OS는 이를 소켓(socket) 인터페이스로 제공하는 경우가 많다.

정보를 요청하는 쪽을 클라이언트, 요청을 받아 처리하고 응답하는 쪽을 서버라고 한다. 한 컴퓨터에서도 상황에 따라 어떤 프로그램은 클라이언트로, 다른 프로그램은 서버로 동작할 수 있다. 즉, 클라이언트/서버는 고정된 장비의 구분이 아니라 역할인 셈이다.
...
<br>
클라이언트 요청에 대한 응답을 제공하는 서버는 먼저 서버 소켓(Server Socket)이라는 특별한 소켓을 생성한다. 서버 소켓은 클라이언트와의 초기 연결을 수행한 후 커널 내 OS backlog queue에 서버와 클라이언트 간의 TCP/IP 연결 정보를 저장한다(이와 동시에 3-way handshake 과정을 거친다).
...
Java는 프로세스가 종료될 때 자원 정리나 로그 기록과 같은 종료 작업을 마무리 할 수 있는 Shutdown Hook 기능을 제공한다. 전체 프로세스를 종료할 때 정리하려는 작업이나 자원을 ShutdownHook 객체의 인자로 등록한 후, \'Runtime.getRuntime().addShutdownHook()\' 경로를 통해 안정적으로 작업을 종료하거나 자원을 회수할 수 있다.
