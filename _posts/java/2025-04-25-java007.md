...
JAVA는 프로세스가 종료될 때 자원 정리나 로그 기록과 같은 종료 작업을 마무리 할 수 있는 <span style="color: rgb(3, 150, 150); font-weight: bold;">`ShutdownHook`</span> 기능을 지원합니다. 셧다운 훅이 실행될 때 처리하려는 작업이나 자원을 ShutdownHook의 인자로 등록한 후 `Runtime.getRuntime().addShutdownHook()` 경로를 통해 스레드를 선언하면 프로세스가 끝나기 전에 안정적으로 작업을 종료하거나 자원을 회수할 수 있습니다.
