...
public class PersonMain {
    public static void main(String[] args) throws Exception {
        ...
        // 클래스 메타데이터 조회 방법 3가지
        Class<? extends Person> aClass = person.getClass(); // 인스턴스로 조회
        // Class<Person> aClass = Person.class; // 클래스로 조회
        // Class<?> aClass = Class.forName("reflection.prac.PersonMain$Person"); // 문자열 조회
...
<br>
심지어 리플렉션을 활용하면 private 접근 제어자에도 직접 접근해서 값을 변경할 수 있다. 하지만 이는 자바가 지향하는 캡슐화 및 유지보수성에 악영향을 미칠 수 있다.

따라서 리플렉션을 사용할 때는 반드시 신중하게 접근해야 하며, 가급적 getter, setter와 같은 접근 메서드를 정의 후 사용하는 것이 차선의 방법이다(getter, setter의 무분별한 사용 또한 캡슐화 원칙을 무너뜨린다).

## 애노테이션
애노테이션(Annotation)은 클래스, 메서드, 변수 등에 메타데이터(Metadata)를 부여하여 컴파일러나 런타임 과정에서 특정 동작을 유도할 수 있도록 돕는 문법이다. 자바 상속에서의 `@Override`, 스프링에서 `@RestController`, `@Transactional` 등을 활용한 프로그래밍이 애노테이션 활용의 대표적 예시다.
...
public class Test {
    public static void main(String[] args) {
        System.out.println(Parent.class.isAnnotationPresent(MyAnno.class));
