...
## 애노테이션
애노테이션(Annotation)은 클래스, 메서드, 변수 등에 메타데이터(Metadata)를 부여하여 컴파일러나 런타임 과정에서 특정 동작을 유도할 수 있도록 돕는 문법이다. 자바에서의 `@Override`, 스프링에서 컨트롤러 `@Controller`를 활용한 프로그래밍이 애노테이션 활용의 대표적 예시다.
