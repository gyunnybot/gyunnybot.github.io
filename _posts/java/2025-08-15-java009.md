...
기본 스트림을 인자로 받아 더 편리한 기능을 제공하는 스트림(이하 보조 스트림)이 몇 가지 있다. 기본 스트림은 항상 바이트 단위로 데이터를 처리하는데, Reader, Writer 스트림은 문자열을 그대로 입력받아 바로 전송할 수 있다.

보내거나 받고 싶은 문자열을 바이트 단위로 변환하는 수고로움을 더는 셈인데, 물론 내부적으로 기본 스트림을 호출하므로 외부 경로나 문자 집합 등의 필요한 정보는 변함없이 입력해야 한다. 예를 들어, 파일에 문자열을 쓰거나 읽을 때는 다음과 같이 사용한다.
