...
## Executor 프레임워크
Runnable을 통해 스레드를 직접 생성하는 경우, 스레드를 생성하고 관리하는 단계에서부터 커널과 CPU 자원을 소모(오버헤드 발생)한다. 또한 Runnable 인터페이스는 반환값이 없기 때문에, 스레드를 통해 값을 도출하려면 변수를 내부적으로 따로 생성한 후 반환해야 한다는 단점이 있다.
...
// Fixed Thread Pool
new ThreadPoolExecutor(nT, nT, 0L,
    TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>());
