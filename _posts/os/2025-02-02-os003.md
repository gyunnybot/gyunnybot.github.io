...
```c
  ...
  void main(){
    ...
    ad.sin_addr.s_addr = inet_addr("127.0.0.1"); //통신할 서버 IP 주소를 루프백 주소로 설정
    ad.sin_port = htons(11234); //클라이언트가 요청한 서버 프로세스의 포트 번호
...
```
<br>
요청에 대한 응답을 제공하는 서버는 먼저 <span style="color: rgb(3, 150, 150); font-weight: bold;">`서버 소켓(Server Socket)`</span>이라는 특별한 소켓을 생성합니다. 이후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`bind()`</span>를 통해 특정 포트 번호를 서버 소켓에 등록합니다. 클라이언트가 요청한 프로세스의 포트 번호가 서버 소켓에 등록된 포트 번호와 동일할 때 해당 서버와 연결이 이루어집니다.

포트 번호가 등록된 서버 소켓은 <span style="color: rgb(3, 150, 150); font-weight: bold;">`listen()`</span>을 실행하여 클라이언트 소켓이 보내는 connect()를 받을 준비를 한 후, <span style="color: rgb(3, 150, 150); font-weight: bold;">`accept()`</span>를 통해 요청받은 여러 클라이언트 connect() 중 하나를 선택합니다.
...
```c
  ...
  void main(){
    ...
    listen(sp, 10); //클라이언트의 conncet() 대기
