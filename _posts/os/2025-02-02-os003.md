...
```
<br>
요청에 대한 응답을 제공하는 서버는 소켓을 생성한 후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`bind()`</span>를 사용해 클라이언트가 요청한 프로세스 포트 번호에 해당 소켓을 등록합니다. 이후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`listen()`</span>을 실행하여 클라이언트 소켓을 받을 준비를 하고 나서 <span style="color: rgb(3, 150, 150); font-weight: bold;">`accept()`</span>를 통해 요청받은 여러 connect() 중 하나를 선택해 본인의 소켓과 연결합니다.
