...
```c
  ...
  void main(){
    ...
    ad.sin_addr.s_addr = inet_addr("127.0.0.1"); //통신할 서버 IP address: 루프백 주소로 설정
    ad.sin_port = htons(11234); //클라이언트가 요청한 프로세스의 포트 번호
...
```
<br>
요청에 대한 응답을 제공하는 서버는 먼저 <span style="color: rgb(3, 150, 150); font-weight: bold;">`서버 소켓(Server Socket)`</span>이라는 특별한 소켓을 생성합니다. 이후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`bind()`</span>를 통해 클라이언트가 요청한 프로세스의 포트 번호로 서버 소켓을 등록합니다.
...
```c
  ...
  void main(){
    ...
    while(true){ //서버의 경우 계속 클라이언트의 요청을 받아 작업해야 하므로 무한 루프
      sa = accept(sp, 0, 0); //여러 클라이언트 connect() 중 하나를 선택 후 sa로 연결
      write(sa, "Test", 0);
      close(sa);
    }
