...
<span style="color: rgb(3, 150, 150); font-weight: bold;">`바쁜 대기(busy waiting)`</span>란 임계구역에 존재하는 자원에 락을 걸고 반복문을 통해 지속적으로 락의 해제를 확인하는 방식입니다. 먼저 반복문에서 해당 자원에 락이 걸려있는지 확인 후, 풀려있다면 잠금을 진행하고 임계 구역에 진입합니다.
...
하지만 반복문과 락 사이에 문맥교환이 발생한다면 다른 프로세스와의 경합이 발생할 수 있으므로, 하드웨어가 지원하는 <span style="color: rgb(3, 150, 150); font-weight: bold;">`검사와 지정(test-and-set)`</span> 방식을 사용해야 합니다.
...
하지만 세마포어 방식 또한 실행 도중 다른 코드가 중간에 삽입된다면 임계구역 문제를 해결할 수 없게 됩니다. 바쁜 대기는 하지 않지만, P()와 V() 코드 블록은 여전히 검사와 지정을 사용해 한번에 실행되도록 해야 합니다.
