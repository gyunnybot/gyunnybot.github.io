...
포트 번호가 등록된 서버 소켓은 <span style="color: rgb(3, 150, 150); font-weight: bold;">`listen()`</span>을 실행하여 클라이언트 소켓을 받을 준비를 한 후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`accept()`</span>를 통해 요청받은 여러 클라이언트 connect() 중 하나를 선택해 클라이언트와 연결합니다.

마지막으로 서버 소켓은 본인이 가지고 있는 포트 번호를 기반으로 새로운 소켓을 생성한 후 연결된 클라이언트와 통신할 수 있도록 돕습니다.
