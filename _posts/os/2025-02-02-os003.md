...
## 소켓 프로그래밍
여러 컴퓨터 간에 정보를 교환하는 기술을 <span style="color: rgb(3, 150, 150); font-weight: bold;">`네트워킹(Networking)`</span>이라고 하며, 일반적으로 네트워크를 통한 프로세스 간 통신(IPC)은 소켓을 활용하기 때문에 <span style="color: rgb(3, 150, 150); font-weight: bold;">`소켓 프로그래밍(Socket Programming)`</span>으로도 불립니다.

정보를 요청한 클라이언트는 소켓을 생성한 후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`connect()`</span>를 통해 서버와의 접속을 시도합니다. 서버와 연결되면 <span style="color: rgb(3, 150, 150); font-weight: bold;">`read()`</span>와 <span style="color: rgb(3, 150, 150); font-weight: bold;">`write()`</span>를 활용해 통신 후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`close()`</span>로 접속을 종료합니다.
...
```
<br>
요청에 대한 응답을 제공하는 서버는 먼저 <span style="color: rgb(3, 150, 150); font-weight: bold;">`서버 소켓(Server Socket)`</span>이라는 특별한 소켓을 생성합니다. 이후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`bind()`</span>를 통해 클라이언트가 요청한 프로세스 포트 번호로 서버 소켓을 등록합니다.

포트 번호가 등록된 서버 소켓은 <span style="color: rgb(3, 150, 150); font-weight: bold;">`listen()`</span>을 실행하여 클라이언트 소켓을 받을 준비를 한 후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`accept()`</span>를 통해 요청받은 여러 클라이언트 connect() 중 하나를 선택합니다.

마지막으로 서버 소켓은 본인이 가지고 있는 포트 번호를 기반으로 새로운 소켓을 생성한 후 선택된 클라이언트 소켓의 connect()와 연결되도록 돕습니다.

정리하면 클라이언트와 서버의 연결 자체는 서버 소켓이 담당하지만, 실제 통신은 서버 소켓에 의해 생성된 소켓과 진행됩니다.
...
```c
  ...
  void main(){
    int sp, sa; //서버(리스닝) 소켓 기술자 sp, 클라이언트와 연결될(된) 소켓 기술자 sa
    struct sockaddr_in ad;
    ...
    bind(sp, (struct sockaddr *) &ad, sizeof(ad)); //특정 포트 번호로 서버 소켓 등록

    listen(sp, 10); //클라이언트 소켓 받을 준비

    while(true){ //서버의 경우 계속 클라이언트의 요청을 받아 작업해야 하므로 while(true)
