...
<span style="color: rgb(3, 150, 150); font-weight: bold;">`바쁜 대기(Busy Waiting)`</span>란 특정 자원의 접근 권한을 얻을 때까지 CPU를 점유한 채 반복문을 지속적으로 실행하여 권한을 확인하는 방식입니다. 반복문을 통해 해당 자원에 락이 걸려있는지 확인 후, 풀려있다면 잠금을 진행하고 임계 구역에 진입합니다.
...
## 소켓 프로그래밍
여러 컴퓨터에 있는 프로세스 간 통신을 <span style="color: rgb(3, 150, 150); font-weight: bold;">`네트워킹`</span>이라고 하며, 일반적으로 소켓을 활용하기 때문에 <span style="color: rgb(3, 150, 150); font-weight: bold;">`소켓 프로그래밍`</span>으로도 불립니다. 소켓은 프로세스 간 양방향 통신과 운영체제 동기화를 지원합니다.
...
```
<br>
한편, 서버는 소켓을 생성 후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`bind()`</span>를 사용해 통신할 프로세스에 해당하는 포트 번호에 등록합니다. 이후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`listen()`</span>을 실행하여 클라이언트 소켓을 받을 준비를 한 후, <span style="color: rgb(3, 150, 150); font-weight: bold;">`accept()`</span>를 통해 요청받은 여러 connect() 중 하나를 선택해 본인의 소켓과 연결합니다.
