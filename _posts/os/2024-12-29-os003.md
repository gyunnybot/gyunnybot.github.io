...
하지만 세마포어 방식 또한 실행 도중 다른 코드가 중간에 삽입된다면 임계구역 문제를 해결할 수 없게 됩니다. 따라서 세마포어 방식은 하드웨어가 지원하는 <span style="color: rgb(3, 150, 150); font-weight: bold;">`검사와 지정(test-and-set)`</span> 명령어를 사용해 P()와 V() 코드 블록마다 한번에 실행되도록 해야 합니다. 검사와 지정은 CPU 단위에서의 원자적 명령어로 실행 중간에 간섭받거나 중단되지 않으며, 같은 메모리 영역에 대해서도 동시에 실행되지 않습니다.
