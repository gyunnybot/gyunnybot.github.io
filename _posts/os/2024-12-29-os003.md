...
```
<br>
서버는 소켓을 생성한 후, <span style="color: rgb(3, 150, 150); font-weight: bold;">`bind()`</span>를 사용해 클라이언트가 요구한 프로세스에 대응하는 특정 포트 번호에 소켓(listening socket)을 등록합니다. 이후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`listen()`</span>을 실행하여 클라이언트를 받을 준비를 한 후 <span style="color: rgb(3, 150, 150); font-weight: bold;">`accept()`</span>를 통해 여러 클라이언트가 요청한 connect() 중 하나를 선택해 포트에 등록된 본인의 소켓과 연결합니다. read(), write()를 통해 통신하며 close()를 통해 클라이언트와의 접속을 종료합니다.
...
```c
  ...
  void main(){
    int sp, sa; //서버가 생성한 소켓 기술자 sp, 클라이언트와 연결될(된) 소켓 기술자 sa
    struct sockaddr_in ad;
    ...
    listen(sp, 10); //listening socket. listen()으로 클라이언트 소켓의 연결 요청을 받는다

    while(true){ //서버의 경우 계속 클라이언트를 받아 작업해야 하므로 무한 루프를 돌려 sa를 갱신한다
      sa = accept(sp, 0, 0);
      write(sa, "Test", 0);
      close(sa);
    }
