...
```c
  ...
  void main(){
    ...
    ad.sin_addr.s_addr = inet_addr("127.0.0.1"); //인터넷이 연결되어 있지 않아도 사용 가능한 IP 주소
    ad.sin_port = htons(11234); //포트 번호
    //통신 초기화 완료

    connect(sp, (struct sockaddr *) &ad, sizeof(ad)); //서버와 접속
...
```c
  ...
  void main(){
    ...
    sp = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP); //클라이언트 소켓 기술자 sp
