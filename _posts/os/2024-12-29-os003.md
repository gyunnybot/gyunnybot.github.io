...
## 임계구역 해결 방법: 세마포어
<span style="color: rgb(3, 150, 150); font-weight: bold;">`세마포어(Semaphore)`</span>는 임계구역 해결 방법 중 변수 자원에 대한 **바쁜 대기(busy wating)**를 하지 않는 알고리즘입니다. <span style="color: rgb(3, 150, 150); font-weight: bold;">`Semaphore()`</span>로 현재 사용 가능한 자원의 수를 선언 후, <span style="color: rgb(3, 150, 150); font-weight: bold;">`P()`</span>와 <span style="color: rgb(3, 150, 150); font-weight: bold;">`V()`</span> 함수로 임계구역을 통제합니다.
...
```c
  ...
  //전역 변수 선언
  int RS; //현재 사용 가능한 자원 개수
  atomic_int guard = 0; //원자적 변수로 lock 구현
  pthread_mutex_t mutex; //세마포어 보호를 위한 뮤텍스
  pthread_cond_t cond; //블로킹된 스레드를 깨우기 위한 조건 변수

  //TestAndSet: 원자적 연산 (lock을 획득하면 1을 반환)
  int TestAndSet(atomic_int *lockPtr) {
      return __sync_lock_test_and_set(lockPtr, 1);
  }
  ...
  //P 연산 (자원 획득)
  void P() {
      while (TestAndSet(&guard)); //스핀락(바쁜 대기) 방식으로 lock 획득. guard = 1

      pthread_mutex_lock(&mutex); //임계구역 보호

      while (RS <= 0) {
          pthread_cond_wait(&cond, &mutex); //자원이 없으면 대기
      }

      RS--; //자원 사용
      printf("자원 사용: 남은 자원 수 = %d\n", RS);

      pthread_mutex_unlock(&mutex); //임계구역 해제
      guard = 0; //lock 해제
  }
  ...
  void V() {
      ...
      RS++; //자원 반환
      printf("자원 반환: 남은 자원 수 = %d\n", RS);

      pthread_cond_signal(&cond); //대기 중인 스레드 깨우기
...
하지만 세마포어 방식 또한 실행 도중 다른 코드가 중간에 삽입된다면 임계구역 문제를 해결할 수 없게 됩니다. 따라서 세마포어 방식은 하드웨어가 지원하는 <span style="color: rgb(3, 150, 150); font-weight: bold;">`검사와 지정(test-and-set)`</span> 명령어 함수를 사용해 P()와 V() 코드 블록이 한번에 실행되도록 해야 합니다.
