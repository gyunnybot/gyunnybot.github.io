...
하지만 세마포어 방식 또한 실행 도중 다른 코드가 중간에 삽입된다면 임계구역 문제를 해결할 수 없게 됩니다. 따라서 세마포어 방식은 하드웨어가 지원하는 <span style="color: rgb(3, 150, 150); font-weight: bold;">`검사와 지정(test-and-set)`</span> 명령어를 사용해 조건을 묶어서 코드 블록이 한번에 실행되도록 해야 합니다.
