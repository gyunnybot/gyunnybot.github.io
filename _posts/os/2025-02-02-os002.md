...
<span style="color: rgb(3, 150, 150); font-weight: bold;">`SRT(Shortest Remaining Time)`</span> 알고리즘은 라운드 로빈을 기본으로 하되, 남은 실행 시간이 가장 적은 프로세스를 우선 실행합니다.
...
요약하면, 사용자 프로세스가 커널 모드로 접근할 수 있는 방법은 두 가지가 있습니다. 운영체제가 제공하는 시스템 호출로 자발적으로 접근하는 방법과 인터럽트를 발생시켜 비자발적으로 접근하는 방법입니다. API는 사용자가 시스템 호출을 편하게 사용할 수 있게 접근을 돕는 역할일 뿐, 자발적으로 커널 모드에 진입할 수 있는 유일한 수단은 **시스템 호출**뿐입니다.