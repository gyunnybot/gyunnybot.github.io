...
## 메모리 직접 접근, 메모리 맵 입출력
초기 컴퓨터 시스템은 CPU가 입출력 명령을 만나면 직접 입출력장치에서 데이터를 가져왔습니다. 입출력장치를 포함한 다양한 주변장치들은 메인보드 내 시스템 버스(FSB)를 사용하기 때문에 내부 버스(BSB)를 사용하는 필수장치보다 속도가 느려 입출력이 끝날 때까지 CPU는 다른 작업을 수행할 수 없었습니다.
...
<span style="color: rgb(3, 150, 150); font-weight: bold;">`시그널`</span>은 사용자가 임의로 발생시키는 인터럽트입니다. \'Ctrl + C\' 명령어처럼 사용자가 발생시키는 자발적인 인터럽트입니다.